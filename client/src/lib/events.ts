// Fix the API_BASE_URL to prevent double slashes
...
export async function fetchEvents(): Promise<Event[]> {
  try {
    ...
    console.log('🔄 fetchEvents: Fetching from:', url);
...
export async function fetchFeaturedEvents(): Promise<Event[]> {
  try {
    ...
    const url = buildApiUrl('/events/featured');
    console.log('🔄 fetchFeaturedEvents: Fetching from:', url);
...
export async function fetchUserRegistrations(userId: string): Promise<any[]> {
  try {
    const response = await fetch(buildApiUrl(`/events/user/${userId}/registrations`));
